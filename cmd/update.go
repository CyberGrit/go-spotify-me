	...
	case tea.KeyMsg:
		...
		case "a", "A":
			...
			if m.currentView == viewMenu {
				...
				return m, func() tea.Msg {
					response, err := fetchArtistsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/artists?time_range=medium_term")
					if err != nil {
						return errMsg{err}
					}
		...
		case "s", "S":
			...
			if m.currentView == viewMenu {
				m.songTable.Focus()
				return m, func() tea.Msg {
					response, err := fetchSongsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/tracks?time_range=medium_term")
					if err != nil {
						return errMsg{err}
					}
		...
		case "1":
			...
			case viewArtists:
				m.artistTable.Focus()
				return m, func() tea.Msg {
					response, err := fetchArtistsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/artists?time_range=short_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{response}
			...
			case viewSongs:
				m.songTable.Focus()
				return m, func() tea.Msg {
					response, err := fetchSongsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/tracks?time_range=short_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{response}
		...
		case "3":
			...
			case viewSongs:
				m.songTable.Focus()
				return m, func() tea.Msg {
					response, err := fetchSongsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/tracks?time_range=long_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{response}
			...
			case viewArtists:
				m.artistTable.Focus()
				return m, func() tea.Msg {
					response, err := fetchArtistsPage(m.spotifyClient, "https://api.spotify.com/v1/me/top/artists?time_range=long_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{response}
		...
		case "right": // Handle next page for Artists or Songs
			if m.currentView == viewArtists && m.artists.Next != "" {
				return m, func() tea.Msg {
					response, err := fetchArtistsPage(m.spotifyClient, m.artists.Next)
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{response}
				...
				return m, func() tea.Msg {
					response, err := fetchSongsPage(m.spotifyClient, m.songs.Next)
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{response}
		...
		case "left": // Handle previous page for Artists or Songs
			if m.currentView == viewArtists && m.artists.Prev != "" {
				return m, func() tea.Msg {
					response, err := fetchArtistsPage(m.spotifyClient, m.artists.Prev)
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{response}
				...
				return m, func() tea.Msg {
					response, err := fetchSongsPage(m.spotifyClient, m.songs.Prev)
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{response}
		...
		case "enter":
			...
			if m.currentView == viewEnterClientID {
				...
				me, err := fetchMe(m.spotifyClient)
				if err != nil {
