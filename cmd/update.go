	...
	case tea.KeyMsg:
		...
		case "a", "A":
			...
			if m.currentView == viewMenu {
				...
				return m, func() tea.Msg {
					artists, next, prev, err := m.client.GetTopArtists("medium_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{APIResponse{Artists: artists, Next: next, Prev: prev}}
				}
			}
		...
		case "s", "S":
			...
			if m.currentView == viewMenu {
				m.songTable.Focus()
				return m, func() tea.Msg {
					songs, next, prev, err := m.client.GetTopSongs("medium_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{APIResponse{Songs: songs, Next: next, Prev: prev}}
				}
			}
		...
		case "1":
			...
			case viewArtists:
				m.artistTable.Focus()
				return m, func() tea.Msg {
					artists, next, prev, err := m.client.GetTopArtists("short_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{APIResponse{Artists: artists, Next: next, Prev: prev}}
			...
			case viewSongs:
				m.songTable.Focus()
				return m, func() tea.Msg {
					songs, next, prev, err := m.client.GetTopSongs("short_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{APIResponse{Songs: songs, Next: next, Prev: prev}}
		...
		case "3":
			...
			case viewSongs:
				m.songTable.Focus()
				return m, func() tea.Msg {
					songs, next, prev, err := m.client.GetTopSongs("long_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{APIResponse{Songs: songs, Next: next, Prev: prev}}
			...
			case viewArtists:
				m.artistTable.Focus()
				return m, func() tea.Msg {
					artists, next, prev, err := m.client.GetTopArtists("long_term")
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{APIResponse{Artists: artists, Next: next, Prev: prev}}
		...
		case "right": // Handle next page for Artists or Songs
			if m.currentView == viewArtists && m.artists.Next != "" {
				return m, func() tea.Msg {
					artists, next, prev, err := m.client.GetArtistsPage(m.artists.Next)
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{APIResponse{Artists: artists, Next: next, Prev: prev}}
				...
				return m, func() tea.Msg {
					songs, next, prev, err := m.client.GetSongsPage(m.songs.Next)
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{APIResponse{Songs: songs, Next: next, Prev: prev}}
		...
		case "left": // Handle previous page for Artists or Songs
			if m.currentView == viewArtists && m.artists.Prev != "" {
				return m, func() tea.Msg {
					artists, next, prev, err := m.client.GetArtistsPage(m.artists.Prev)
					if err != nil {
						return errMsg{err}
					}
					return switchToArtistsMsg{APIResponse{Artists: artists, Next: next, Prev: prev}}
				...
				return m, func() tea.Msg {
					songs, next, prev, err := m.client.GetSongsPage(m.songs.Prev)
					if err != nil {
						return errMsg{err}
					}
					return switchToSongsMsg{APIResponse{Songs: songs, Next: next, Prev: prev}}
