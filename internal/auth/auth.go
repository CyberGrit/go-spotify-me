...
func GenerateCodeVerifier() string {
	...
	_, err := rand.Read(verifier) // Use crypto/rand for secure random bytes
	if err != nil {
...
func ExchangeCodeForToken(authConfig AuthConfig, code, codeVerifier string, store TokenStore) {
	...
	// Save tokens using the TokenStore
	if err := store.SaveTokens(accessToken, refreshToken, expirationTime); err != nil {
...
func RefreshAccessToken(authConfig AuthConfig, refreshToken string, store TokenStore) error {
	...
	// Save the new access token using the TokenStore
	// Assuming 1 hour expiration for refresh
	if err := store.SaveTokens(accessToken, refreshToken, time.Now().Add(3600*time.Second)); err != nil {
